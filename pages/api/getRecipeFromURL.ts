...
export default async function playwright(req: NextApiRequest, res: NextApiResponse) {
...
async function getScriptContent(url: string) {
    ...
    const scriptContent: string | null = await page.$eval('script[type="application/ld+json"]', (element) => element.textContent);
